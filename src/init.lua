...
function Module.Text(self: ToolTip, value: string?): string
	if value then (self.instance :: TextLabel).Text = value end -- sad type casting noises
...
function Module.Enable.Gui(self: GuiToolTip)
	if Module.IsEnabled.Gui(self) then return end
...
function Module.Enable.PV(self: PVToolTip)
	if Module.IsEnabled.PV(self) then return end
...
function Module.Disable.Gui(self: GuiToolTip)
	if not Module.IsEnabled.Gui(self) then return end
...
function Module.Disable.PV(self: PVToolTip)
	if not Module.IsEnabled.PV(self) then return end
...
function Module.Add.PV(self: PVToolTip, pv: PVInstance)
	if self.pvs[pv] then return end
	self.pvs[pv] = true
end
...
function Module.Remove.Gui(self: GuiToolTip, gui: GuiObject)
	...
	if Module.IsEnabled.Gui(self) then disconnectGui(self, gui) end
	self.guis[gui] = nil
end

function Module.Remove.PV(self: PVToolTip, pv: PVInstance)
	if not self.pvs[pv] then return end
	self.pvs[pv] = nil
end
...
local function fromPV(gui: GuiObject, offset: Vector2, anchor: Vector2, distance: number?, raycastParams: RaycastParams?)
	gui.AnchorPoint = anchor
...
function Module.From.PV(gui: GuiObject, offset: Vector2, anchor: Vector2, distance: number?, raycastParams: RaycastParams?)
	gui.Visible = false
	return fromPV(gui, offset, anchor, distance, raycastParams)
end

function Module.From.PVText(text: string, offset: Vector2, anchor: Vector2, distance: number?, raycastParams: RaycastParams?): PVToolTip
	local TextLabel = Module.DefaultTextLabel:Clone()
	TextLabel.Text = text
